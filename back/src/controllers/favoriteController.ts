...
import { searchFavorite, addFavorite, deleteFavorite } from '../services/favoriteService';

export const favoriteSwitch = async (req : IRequest, res : Response, next : NextFunction) => {
    try {
        // req를 통해 넘어온 diaryId
        ...
        if (like) {
            ...
            return res.status(200).json({ message: '해당 일기의 좋아요를 취소했어요.' });
